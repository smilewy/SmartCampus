    ...
    final function download($filename,$aName,$single=false)
    {
        if(!$single){
            ...
            for ($i = 0; $i < count($filename); $i++) {
                ...
                $zip->renameName($this->rootPath .$filename[$i],$aName[$i]);
            }
            $zip->close();//关闭
            $file_name = $zipname . '.zip';
            $filepath =  './Public/upload/' . $file_name;
        }else{ //单个文件
            $filepath =  './Public/upload/' . $filename[0];
            $file_name = $aName[0];
        }
...
//每次向客户端回送1024个字节的数据
    ...
    public function update($preFile, $upload, $prefix = '')
    {
        ...
        if (empty($upload)) {//没有附件 将原来的删除
            ...
            //有附件
            //求差集
            $difference = array_diff($preFile,$upload);//求变化的文件名
            if (empty($difference)) {//附件没有改变
                ...
                $nextFile = array_merge($upload, $newFile['path']);
            } else {
                ...
                $nextFile = array_intersect($upload, $newFile['path']);
            }
        }
        return $nextFile;
    }
    ...
    public function uploadExcel(){
        ...
        if (! empty ($this->file))
        {
            $tmp_file = $_FILES ['import'] ['tmp_name']; //上传文件必须命名为import
