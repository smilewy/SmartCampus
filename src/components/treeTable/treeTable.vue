...
//        console.log(column)
      ...
      recursiveNode( obj ){
        if( 'childs' in obj ){
          obj.childs.forEach( o => {
            o.ifUser = obj.ifUser;
            this.recursiveNode( o );
          });
        }
      },

      // 勾选了子分类后将父分类也设置为勾选，取消时判断其父分类是否存在已勾选的子分类，有则仍为勾选状态，无则取消勾选
      bubbleNode( rowData ) {
          const parent = rowData._parent;
          if( parent ) {
              // 如果不是勾选状态，判断其父分类是否存在其他子分类为勾选状态
              if( !rowData.ifUser ){
                  if( !parent.childs.some( o => o.ifUser ) ){
                      parent.ifUser = rowData.ifUser;
                  }
              } else {
                  parent.ifUser = rowData.ifUser;
              }
              this.bubbleNode( parent );
          }
      },
      ...
      useTypeChange(rowI){
        let params={};
        let rowData = this.tableData[rowI];
        this.ParamObj.forEach((value)=>{
          if(value in rowData){
        ...
        this.recursiveNode( rowData );
        this.bubbleNode( rowData );
        /*检测勾选或不勾选全选checkbox框*/
        //this.checkChildChoose()
        this.checkChooseAll();
