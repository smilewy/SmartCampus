<template>
  ...
  <div>
    ...
    <el-table class="g-NotHover" v-else ref="treeTable1" :data="tableData" style="width:100%" @sort-change="sortChange"
              :row-style="showTr">
...
//        console.log(column)
      ...
      checkChildChoose(obj){
        ...
        obj.forEach((value,index)=>{
          _self.$refs.treeTable1.toggleRowSelection(_self.tableData[Number(value._index)],_self.isCheckAll);
          if(value.childs){
      ...
      showTr(row, index){
        let obj = row.row||{},
            show = (obj._parent ? (obj._parent._expand && obj._parent._show) : true);
        obj._show = show;
