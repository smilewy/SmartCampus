  ...
  <div class="newExam">
        ...
        <el-row class="newExam_processSpec">
          <el-form :inline="true" :rules="rules" :model="rulesData" ref="ruleForm">
            <el-form-item label="年级：">
              <el-select class="grade" v-model="selectParam.gradeid" placeholder="请选择"
            ...
            <el-form-item label="考试名称：" prop="examination">
              <el-input class="examName" :maxlength="10" v-model="rulesData.examination"
                        placeholder="请输入考试名称,1-10个字符"></el-input>
            </el-form-item>
            <el-form-item label="时间段：" required>
              <el-row class="tTime">
                <el-col :span="11">
                  <el-form-item prop="startTime">
                    <el-date-picker :editable="false" type="date" placeholder="选择日期时间"
                                    :picker-options="pickerBeginDateBefore" v-model="rulesData.startTime"
                ...
                <el-col :span="11">
                  <el-form-item prop="endTime">
                    <el-date-picker :editable="false" type="date" placeholder="选择日期时间"
                                    :picker-options="pickerBeginDateAfter" v-model="rulesData.endTime"
                                    style="width: 100%;"></el-date-picker>
                  </el-form-item>
                </el-col>
              </el-row>
      ...
      <el-row class="newExam_process">
        ...
        <el-row class="newExam_processBody">
          <el-form :inline="true" class="templating">
            <el-form-item label="历次考试科目模板：">
              <el-select v-model="selectParam.examinationid" placeholder="请选择"
        ...
        <el-row :gutter="30" class="newExam_processBody" v-for="(subject,index) in subjectlist" :key="subject.branchid">
          ...
          <el-col :span="20" class="newExam_subject">
            <el-button v-for="(data,idx) in subject.subjectlist" :key="data.subjectid" :class="{'active':data.state}"
    ...
    <div class="stepsThree" v-show="examIdx==3">
      <el-row class="newExam_process">
        ...
        <el-row :gutter="30" class="newExam_processBody" v-for="(data,indx) in classlist" :key="data.branch">
          <el-col :span="4">
            <el-row class="arts">{{data.branch}}：</el-row>
            <el-row>
              <el-col :span="8" class="choose"><span @click="checkedList('all',indx,'classlist')">全选</span></el-col>
              <el-col :span="8" class="choose anti-election"><span
                @click="checkedList('anti',indx,'classlist')">反选</span></el-col>
              <el-col :span="8" class="choose"><span @click="checkedList('clear',indx,'classlist')">清空</span></el-col>
            </el-row>
          </el-col>
          <el-col :span="20" class="newExam_subject">
            <el-button v-for="(classes,idx) in data.classlist" key="idx" :class="{'active':classes.state}"
                       @click="chooseSubject(indx,idx,'classlist')">{{classes.classname}}班
            </el-button>
          </el-col>
        </el-row>
      </el-row>
    ...
    data() {
      return {
        ...
        pickerBeginDateBefore: {
          disabledDate: (time) => {
            let beginDateVal = this.rulesData.endTime;
            if (beginDateVal) {
        ...
        pickerBeginDateAfter: {
          disabledDate: (time) => {
            let beginDateVal = this.rulesData.startTime;
            if (beginDateVal) {
              return time.getTime() < beginDateVal;
            }
          }
    ...
    methods: {
      ...
      nextSteps(idx) {
          ...
          self.$refs['ruleForm'].validate((valid) => {
            if (valid) {
              ...
              for (let obj of self.subjectlist) {
                ...
                if (branch.subjectlist.length != 0) {
                  self.form.data.push(branch);
                }
              }
              if (self.form.data.length == 0) {
                self.vmMsgWarning('请选择科类科目！');
                return false
              }
              self.examIdx = idx;
        ...
        if (idx == 3) {
          ...
          for (let obj of self.tableData.data) {
            param.branchid.push(obj.branchid);
            for (let mbj of obj.subjectlist) {
              if (!mbj.fullcredit) {
                self.vmMsgWarning('请填写单科满分！');
                return false
              }
              if (!mbj.proportion) {
                self.vmMsgWarning('请填写计入总分分数！');
                return false
              }
            }
          }
      ...
      saveMsg() {
        ...
        self.form.starttime = moment(self.rulesData.startTime).format('YYYY-MM-DD');  //格式化时间
        self.form.endtime = moment(self.rulesData.endTime).format('YYYY-MM-DD');
        ...
        if (self.form.class.length == 0) {
          self.vmMsgWarning('请选择考试班级！');
          return false
        }
