<template>
  <div class="g-statisticalAnalysis g-container compareLine">
    <header>
      <h2>上线对比</h2>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getTableLoad(){
        this.loading=true;
        compareLineLoad({page:this.currentPage,limit:this.pageCount,find:this.fuzzyInput,...this.orderBy,...this.repairForm}).then(data=>{
          this.loading=false;
          if(data.score){
            ...
            this.vmMsgError('两次划线名称不一致！');
          }
        });
      },
      exportClick(){
        if(this.repairForm.examinationid1 && this.repairForm.examinationid2){
          let exportParam='?examinationid1='+this.repairForm.examinationid1+'&examinationid2='+this.repairForm.examinationid2;
          req.downloadFile('.g-container','/school/Achievement/score/type/onlineContrastExport'+exportParam,'post');
        }
        else{
          this.vmMsgWarning('请选择所有相关信息！');
        }
      ...
      operationData(type){
        ...
        if (type == 'copy') {
          req.copyTableData('.compareLine', sAy);
        } else {
          req.lodop(sAy);
        }
      }
