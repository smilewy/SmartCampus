<template>
  <div class="g-statisticalAnalysis g-container totalScoreLine">
    <header>
      <h2>总分上线</h2>
    ...
    <section class="centerTable alertsList">
      ...
      <el-table class="g-NotHover" v-if="isTableShow" border :data="classesTimeSetTable" @sort-change="sortChange"
        ...
        <el-table-column label="班主任/年级主任" prop="teacher" sortable="custom"></el-table-column>
      </el-table>
    </section>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getTableLoad(){
        this.loading=true;
        totalScoreLineLoad({page:this.currentPage,limit:this.pageCount,find:this.fuzzyInput,...this.orderBy,...this.repairForm}).then(data=>{
      ...
      exportClick(){
        if(this.repairForm.examinationid){
          ...
          req.downloadFile('.g-container','/school/Achievement/score/type/totalscoreexport'+exportParam,'post');
        }
        else{
          this.vmMsgWarning('请选择考试！');
        }
      ...
      operationData(type){
        ...
        if (type == 'copy') {
          req.copyTableData('.totalScoreLine', sAy);
        } else {
          req.lodop(sAy);
        }
      }
