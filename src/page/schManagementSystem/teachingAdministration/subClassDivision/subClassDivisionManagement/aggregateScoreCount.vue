  ...
  <div class="subScoreBasis">
    <el-row type="flex" align="middle" class="subClassDivision_title">
      ...
      <span class="breadcrumb"><span class="breadcrumb_active">成绩汇总</span><span>|</span><router-link
        :to="{name:'scoreDetails',params:{planId:selectParam.planId}}"
    ...
    <el-row class="alertsList">
      ...
      >
        ...
        <el-table-column
          :label="headData.wishName" v-for="headData in tableHeadData" :key="headData.wishId">
          <el-table-column
            label="分数">
            <template slot-scope="scope">
              {{scope.row[headData.wishId].score}}
            </template>
          </el-table-column>
          <el-table-column
            label="排名">
            <template slot-scope="scope">
              {{scope.row[headData.wishId].rank}}
            </template>
          </el-table-column>
        </el-table-column>
      </el-table>
  ...
  export default{
    ...
    methods: {
      ...
      operationData(type) {
        ...
        let tableThTr = '<tr><th rowspan="2">年级</th><th rowspan="2">班级</th><th rowspan="2">姓名</th>',
          tableThTr1 = '<tr>', tableTd = '';
        for (let obj of this.tableHeadData) {
          tableThTr += '<th colspan="2">' + obj.wishName + '</th>';
          tableThTr1 += '<th>分数</th><th>排名</th>';
        }
        for (let obj of this.tableData) {
          let tTr = '<tr>';
          tTr += '<td>' + obj.preGrade + '</td><td>' + obj.preClass + '</td><td>' + obj.name + '</td>';
          for (let mbj of this.tableHeadData) {
            tTr += '<td>' + (obj[mbj.wishId].score || '') + '</td><td>' + (obj[mbj.wishId].rank || '') + '</td>'
          }
          tTr += '</tr>';
          tableTd += tTr;
        }
      ...
      loadData(data){
        ...
        req.ajaxSend('/school/DivideBranch/scoreStatistics', 'post', data, function (res) {
          ...
          self.tableHeadData = res.data.wish;
...
<style>
  ...
  .subScoreBasis .el-table--border td {
    border-right: 0;
  }
