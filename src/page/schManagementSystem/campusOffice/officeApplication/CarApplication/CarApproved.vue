    ...
    <el-row>
      <el-col :span="17" class="alertsBtn">
        <el-button-group>
          ...
          <el-button class="delete" title="删除" @click="deleteCarApproved">
      ...
      <el-row class="alertsList">
        <el-table
          ...
          <el-table-column
            ...
            align="center">
            <template  slot-scope="scope">
              <span style="color:#89bcf5;cursor: pointer;" @click="CarApprovedDetails(scope.row)">详情</span>
            </template>
          </el-table-column>
        </el-table>
      </el-row>
      <el-dialog title="用车已审批详情" v-if="dialogTableVisible" :modal="false" :visible.sync="dialogTableVisible">
        <el-row>
  ...
  export default{
    ...
    methods:{
      operationData(type){
        ...
        if (type === 'copy') {
          req.copyTableData('.CarApproved', sAy);
        } else {
          req.lodop(sAy);
        }
      },
      download(){
        req.downloadFile('.CarApproved','/school/WorkDemand/approveCar?export=ensure&whether=2','post');
      },
      handleIconClick(){
        this.querryData(1);
      },
      CarApprovedDetails(row){
        this.link='';
        this.person='';
        let param={
          type:'detail',
