...
<script>
  ...
  export default {
    ...
    created: function () {
      var self = this;
      self.loading = true;
      req.ajaxSend('/school/Studentleave/getLeaveList?type=getLeaveList', 'get', self.selectParam, function (res) {
      ...
      operationTable(type) {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            if (type == 'out') {
              req.downloadFile('.leaveRecords', '/school/Studentleave/getLeaveList?type=export&startTime=' + this.selectParam.startTime + '&endTime=' + this.selectParam.endTime, 'post')
            } else if (type == 'copy') {
              req.copyTableData('.leaveRecords', sAy);
            } else {
              req.lodop(sAy);
            }
          } else {
...
<style>
  ...
  .leaveRecords .createTime .el-form-item {
    margin-right: 0;
  }
