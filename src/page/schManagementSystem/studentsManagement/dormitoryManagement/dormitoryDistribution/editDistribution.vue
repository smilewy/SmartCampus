<template>
  <div class="editDistribution">
    ...
    <el-row class="editDistribution_body">
      <el-form ref="form" :rules="rules" :model="form" label-width="150px" class="subMsg">
        ...
        <el-form-item label="宿舍分配年级：" prop="gradeId">
          <el-select v-model="form.gradeId" :disabled="true" placeholder="请选择" style="width: 100%;">
...
<script>
  ...
  export default{
    ...
    created: function () {
      ...
      req.ajaxSend('/school/StudentDorm/onePlan', 'post', data, function (res) {
        self.form.gradeId = res.data.gradeId == 0 ? 'all' : res.data.gradeId;
        self.form.name = res.data.name;
        self.form.notice=res.data.notice;
      })
    },
    methods: {
      returnFlowchart(){
...
<style>
  .editDistribution .editDistribution_body {
    margin: 4.375rem 0;
  }

  .editDistribution .subMsg {
    width: 75%;
    margin: auto;
  }

  .editDistribution .line {
    text-align: center;
  }

  .editDistribution .noticeUeditor {
    height: 25rem;
  }

  .editDistribution .submitBtn {
    text-align: right;
    margin-top: 2rem;
  }

  .editDistribution .submitBtn .el-button {
    width: 7.5rem;
    padding: 10px 0;
    border-radius: 20px;
  ...
  .editDistribution .submitBtn .el-button--primary {
    color: #fff;
  }

  .editDistribution .vuEditor .el-form-item__content {
    line-height: 1;
  }
</style>
