<template>
  <div class="liveSchool">
    <el-row type="flex" align="middle" class="studyingWayApproval_row">
      <el-button type="primary" class="uploadTemplate" @click="changeState">调整为走读</el-button>
    </el-row>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      operationData(type){
        if (type == 'out') {
          req.downloadFile('.liveSchool', '/school/StudentDorm/attendApprove?download=ensure&option=2', 'post');
        }
      },
      changeState(){  //是否通过审批
        var self = this, data = {
