<template>
  <div class="pendingApproval">
    <el-row type="flex" align="middle" class="studyingWayApproval_row">
      <el-button type="primary" class="uploadTemplate" @click="changeState('pass')">审批通过</el-button>
      <el-button type="danger" class="uploadTemplate" @click="changeState('reject')">审批不通过</el-button>
      <div class="warmHint">
        <el-col :span="2" class="notes">备注：</el-col>
        <el-col :span="22">
          <el-row>审批通过：原住校学生数据转为走读，原走读学生数据转为住校。</el-row>
          <el-row>审批不通过：原住校学生或原走读学生数据仍然保留在原处。</el-row>
        </el-col>
      </div>
    </el-row>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      operationData(type){
        if (type == 'out') {
          req.downloadFile('.pendingApproval', '/school/StudentDorm/attendApprove?download=ensure&option=1', 'post');
        }
      },
      changeState(type){  //是否通过审批
        var self = this, data = {
          option: 1,
          type: type,
          ids: []
