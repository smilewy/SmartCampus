  ...
  export default{
    ...
    methods:{
      operationData(type){
        ...
        if (type === 'copy') {
          req.copyTableData('.GradeStatistics', sAy);
        } else {
          req.lodop(sAy);
        }
      },
      download(){
        ...
        req.downloadFile('.GradeStatistics','/school/StudentEvaluate/statisticsEvaluate?export=ensure&option=grade&gradeId='+gradeId+'&evaId='+evaId,'post');
      },
      getclass(){
        let param={
          func:'getAllEva'
      ...
      getList(page){
        ...
        req.ajaxSend('/school/StudentEvaluate/statisticsEvaluate','post',param,(res)=>{
          ...
          if(this.key){
            this.tableData = res.data.filter(val=>val.subject.indexOf(this.key)>-1);
          }else{
