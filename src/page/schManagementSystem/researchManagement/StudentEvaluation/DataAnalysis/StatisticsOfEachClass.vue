<template>
  <div class="StatisticsOfEachClass">
    <h3>科目各班统计</h3>
    <el-row>
  ...
  export default{
    ...
    methods:{
      operationData(type){
        ...
        if (type === 'copy') {
          req.copyTableData('.StatisticsOfEachClass', sAy);
        } else {
          req.lodop(sAy);
        }
      },
      download(){
        ...
        req.downloadFile('.StatisticsOfEachClass','/school/StudentEvaluate/statisticsEvaluate?export=ensure&option=subjectClass&gradeId='+gradeId+'&subjectId='+subjectId+'&evaId='+evaId,'post');
      },
      getclass(){
        let param={
          func:'getAllEva'
      ...
      getList(page){
        ...
        req.ajaxSend('/school/StudentEvaluate/statisticsEvaluate','post',param,(res)=>{
          ...
          if(this.key){
            this.tableData = res.data.filter(val=>val.name.indexOf(this.key)>-1||val.class.indexOf(this.key)>-1);
          }else{
