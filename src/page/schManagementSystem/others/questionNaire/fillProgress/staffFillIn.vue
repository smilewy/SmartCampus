  ...
  export default{
    ...
    methods:{
      ...
      getTableLoad(){
        if(this.questionId){
          this.loading = true;
          questionNaireRecordProgress({roleNameId:3,questionId:this.questionId,page:this.currentPage,pageSize:this.pageCount,valueData:this.fuzzyInput,...this.orderBy}).then(data=>{
            this.loading = false;
            if(data.statu){
