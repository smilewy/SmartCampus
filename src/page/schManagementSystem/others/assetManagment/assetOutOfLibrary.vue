  ...
  <div class="g-classSchedule">
    <header class="g-timeHeader g-flexStartRow">
      ...
      <ul class="changeRouter selfCenter clear_fix">
        <li class="normalCss" :class="{'activeCss':idx!=1}" @click="changeRouterClick(0)">新增出库</li>
        <li class="normalCss" :class="{'activeCss':idx==1}" @click="changeRouterClick(1)">出库记录</li>
      </ul>
    </header>
    ...
    methods:{
      ...
      changeRouterClick(idx){
        ...
        if (idx == 0) {
          ...
          this.$router.push({name: 'outRecord', params: {idx: idx}});
        }
      }
    },
    created(){
