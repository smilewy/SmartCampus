    ...
    <section class="centerTable alertsList">
      ...
      <el-table class="g-NotHover"
        ...
        <el-table-column label="操作" width="150px" fixed="right">
          <template slot-scope="props">
            <el-button v-if="props.row.state==2" @click="acceptHandler(props.row.id)" type="text">验收</el-button>
            <el-button @click="applicationAgain(props.row.id)" class="deleteColor" type="text">重新申请</el-button>
          </template>
        </el-table-column>
      </el-table>
    </section>
    <el-dialog class="headerNotBackground" :modal="false" title="重新申请说明" :visible.sync="isDialog" :before-close="handlerClose">
      <el-form :model="handlerForm" label-width="100px" label-position="right" :rules="formRulesRexplain">
        <el-form-item label="报修内容:" required prop="restartExplain">
          <el-input v-model="handlerForm.restartExplain"></el-input>
        </el-form-item>
      </el-form>
      <div class="g-button">
        <el-button @click="submitAgain" type="primary">提交</el-button>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      applicationAgain(id){
        this.isDialog=true;
        this.handlerForm.id=id;
        this.handlerForm.restartExplain='';
      },
      /*模糊查询*/
      fuzzyClick(){
        /*模糊查询执行回调*/
      ...
      getLoadAjax(){
        ...
        repaymentLoad({repairType:'list',...this.repairForm,state:2,...this.orderParam,valueData:this.fuzzyInput}).then(data=>{
      ...
      /*验收*/
      acceptHandler(id){
        this.$confirm('确定验收该报修单？','提示',{
          confirmButtonText:'确定',
          type:'warning',
        }).then(()=>{
          repaymentAccept({id:id}).then(data=>{
            this.vmMsgSuccess( data.message || '验收成功！' );
			this.getLoadAjax();
          })
        }).catch(()=>{});
      },
      /*重新申请——确定*/
      submitAgain(){
        if(this.handlerForm.restartExplain){
          this.isDialog=false;
          repaymentAgain(this.handlerForm).then(data=>{
            this.vmMsgSuccess( data.message || '重新申请成功！' );
			this.getLoadAjax();
          });
        }
        else{
          this.vmMsgWarning( '请填写报修内容！' );
        }
      },
    },
    created(){
