  ...
  <div class="g-statisticalAnalysis g-container repairpending">
    ...
    <section class="centerTable alertsList">
      ...
      <el-table class="g-NotHover" 
        ...
        <el-table-column label="维修地点" min-width="150" prop="repairAddress" sortable="custom"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right">
          <template slot-scope="props">
            <el-button v-if="!Number(props.row.repairState)" @click="orderAjax(props.row.id)" type="text">接单</el-button>
            <el-button @click="handlerTaskClick(props.row.id)" v-else class="deleteColor" type="text">处理任务</el-button>
          </template>
        </el-table-column>
      </el-table>
    </section>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      /*处理任务*/
      handlerTaskClick(id){
        this.$router.push({name:'handlerTask',params:{id:id}});
      },
      ...
      getLoadAjax(){
        ...
        serviceTaskLoad({repairType:'list',...this.repairForm,state:1,...this.orderParam,valueData:this.fuzzyInput}).then(data=>{
      ...
      orderAjax(id){
        this.$confirm("确认接收该维修单吗？", "提示", {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then( () => {
          serviceTaskOrder({id:id}).then(data=>{
            this.vmMsgSuccess( '接单成功！' );
			this.getLoadAjax();
          });
        }).catch( () => {});
      },
