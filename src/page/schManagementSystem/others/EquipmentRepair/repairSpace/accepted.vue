<template>
  <div class="g-statisticalAnalysis g-container repairaccepted">
      ...
      <el-form :model="repairForm" label-width="85px" label-position="left">
        <el-form-item label="报修类别:" required>
          <el-select v-model="repairForm.id">
            <el-option v-for="(content,index) in repairTypeData" :key="index" :value="content.id"
                       :label="content.repairType"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="时间段:">
          <el-col :span="10">
            <el-date-picker type="date" :picker-options="pickerOptionStart" placeholder="选择日期"
                            v-model="repairForm.startTime" style="width:100%"></el-date-picker>
          </el-col>
          <el-col :span="2" class="line" style="text-align:center;">--</el-col>
          <el-col :span="10">
            <el-date-picker type="date" :picker-options="pickerOptionEnd" placeholder="选择日期"
                            v-model="repairForm.endTime" style="width:100%"></el-date-picker>
          </el-col>
        </el-form-item>
      </el-form>
      ...
      <div class="g-liOneRow g-sa_header_search">
        ...
        <div class="gs-refresh g-fuzzyInput">
          <el-input type="text" v-model="fuzzyInput" suffix-icon="el-icon-search" placeholder="请输入"
                    @change="searchTableClick"></el-input>
        </div>
      </div>
      <el-table class="g-NotHover" 
                v-loading="loadingAccepted" 
        ...
        <el-table-column label="验收时间" min-width="150" prop="acceptanceTime" sortable="custom"></el-table-column>
        <el-table-column label="维修结果" prop="state" min-width="150" sortable="custom">
    ...
    <el-dialog class="overflowDialog headerNotBackground" :modal="false" title="维修详情" :visible.sync="isDetailDialog"
               :before-close="handlerDetailClose">
      ...
      <div class="g-repairPerson g-liOneWrapRow">
        ...
        <div class="g-detailRow">
          <span class="g-detailLable">验收时间:</span>
          <span v-text="detailForm.acceptanceTime"></span>
        </div>
        <div class="g-detailRow">
  ...
  export default {
    ...
    methods: {
      operationData(type){
        ...
        for (let obj of this.classesTimeSetTable) {
          ...
          for (let name in hdData) {
            if( name == 'state' ){
              d[name] = obj[name] == 1 ? '待处理' : obj[name] == 2 ? '已维修' : obj[name] == 3 ? '维修失败' : obj[name] == 4 ? '已验收' : '未知';
            } else {
              d[name] = obj[name] || '';
            }
          }
        ...
        if (type == 'copy') {
          req.copyTableData('.repairaccepted', sAy);
        } else {
          req.lodop(sAy);
        }
      },
      ...
      getLoadAjax() {
        ...
        repairSpaceLoad({
          repairType: 'list', ...this.repairForm,
          state: 4, ...this.orderParam,
          valueData: this.fuzzyInput
        }).then(data => {
      ...
      exportClick() {
        if (this.repairForm.id) {
          let urlPrompt = '&repairType=export&state=4&id=' + this.repairForm.id;
          req.downloadFile('.g-container', '/school/Eqrepair/repairList?type=getRepairList' + urlPrompt, 'post');
        }
        else {
          this.vmMsgWarning( '请选择报修类别！' );
        }
...
<style lang="less" scoped>
  ...
  .g-statisticalAnalysis header.g-textHeader.g-flexStartRow {
    .marginTop(20);
    .marginBottom(20);
  ...
  .g-textHeader .el-form-item:nth-of-type(1) {
    .width(222, 825);
  }

  .g-textHeader .el-form-item:nth-of-type(2) {
    .width(558, 825);
    .marginLeft(40, 825);
  }
  ...
  /*维修详情弹框*/
  .g-repairDetail {
    border-bottom: 2/16rem solid @borderColor;
    .marginBottom(20);
  }

  .g-liOneWrapRow .g-detailRow {
    .widthRem(370);
  }
