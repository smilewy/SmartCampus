...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      addMsg(type, val){
        if (type == 0) {
          this.$router.push({name: 'workerMessageAddOrEdit', params: {type: 0, id: 1}});
        } else {
          this.$router.push({name: 'workerMessageAddOrEdit', params: {type: 1, id: this.tableData[val].id}});
        }
      },
      deleteAlerts(){
        var self = this;
        ...
        self.$confirm('是否删除职工信息（删除后不能恢复）?', '提示', {
