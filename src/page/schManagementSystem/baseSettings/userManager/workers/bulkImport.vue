    ...
    <el-row class="alertsList">
      ...
      >
        ...
        <el-table-column
          ...
          label="户口所在地">
          <template slot-scope="scope">
            <span>{{scope.row.registerAddress.province}} {{scope.row.registerAddress.city}} {{scope.row.registerAddress.area}}</span>
          </template>
        </el-table-column>
        <el-table-column
          min-width="150"
          label="家庭住址">
          <template slot-scope="scope">
            <span>{{scope.row.homeAddress.province}} {{scope.row.homeAddress.city}} {{scope.row.homeAddress.area}}</span>
          </template>
        </el-table-column>
        <el-table-column
          min-width="150"
          label="现住地址">
          <template slot-scope="scope">
            <span>{{scope.row.nowAddress.province}} {{scope.row.nowAddress.city}} {{scope.row.nowAddress.area}}</span>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    ...
    methods: {
      ...
      operationData(type){
        ...
        for (let obj of this.tableData) {
          ...
          for (let name in hdData) {
            if (name == 'origin') {
              ...
              d[name] = obj[name].province || '' + ' ' + obj[name].city || '' + ' ' + obj.registerAddress.area || '';
            } else {
              d[name] = obj[name] || '';
            }
          }
        ...
        if (type == 'downloadTemplate') {
          req.downloadFile('.bulkImport', '/school/user/userGl?type=download&downType=worker', 'post');
        } else if (type == 'upload') {
          ...
          req.ajaxFile('/school/user/userGl?type=export&roleNameEn=zg', 'post', formData, function (res) {
            if (res.statu == 1) {
          ...
          req.downloadFile('.bulkImport', '/school/user/exportTeacherOrZg?type=worker', 'post');
        } else if (type == 'copy') {
