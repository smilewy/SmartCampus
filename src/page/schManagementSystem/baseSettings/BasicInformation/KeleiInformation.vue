<template>
  <div class="KeleiInformation">
    ...
    <el-col :span="24">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span style="color:#ff6a6a;cursor: pointer;" v-if="scope.row.state===true"  @click="Delectlist(scope.row)">删除</span>
            </template>
          </el-table-column>
        </el-table>
      </el-row>
      <el-dialog title="添加科类" v-if="dialogTableVisibleother" :modal="false" :visible.sync="dialogTableVisibleother">
        <el-row>
          <el-col style="min-height:13rem;">
            <el-col :span="24">
              <el-col :span="5" :offset="2" style="padding-top: .4rem;"><span style="color: red;">*</span> 科类：</el-col>
              <el-col :span="15">
                <el-input placeholder="请输入" v-model="param.branch"></el-input>
              </el-col>
            </el-col>
      ...
      <el-dialog title="编辑科类" v-if="editKeilei" :modal="false" :visible.sync="editKeilei">
        <el-row>
          <el-col style="min-height:13rem;">
            <el-col :span="24">
              <el-col :span="5" :offset="2" style="padding-top: .4rem;"><span style="color: red;">*</span> 科类：</el-col>
              <el-col :span="15">
                <el-input placeholder="请输入" v-model="editData.branch"></el-input>
              </el-col>
            </el-col>
            <el-col :span="2" :offset="11">
...
<script>
  ...
  export default{
    ...
    methods:{
      Default(row){
        if(row.state===false){
          this.vmMsgWarning('该科类信息不可修改');
        }
      },
      GetKeleiList(){
        this.isLoading=true;
        req.ajaxSend('/school/Bcinformation/branch/type/branchselect','post',{order:this.param.order,field:this.param.field},(res)=>{
          if (res.status === -1) {
            this.tableData = [];
      ...
      SaveaddKeilei(){
        ...
        req.ajaxSend('/school/Bcinformation/branch/type/branchupdate','post',param,(res)=>{
          if(res.return===true){
      ...
      Delectlist(row){
        this.state=row.state;
        this.$confirm('确定删除该科类吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          req.ajaxSend('/school/Bcinformation/branch/type/branchdel','post',{branchid:row.branchid},(res)=>{
            if(res.return===true){
